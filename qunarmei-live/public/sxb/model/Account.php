...
class Account
{
    ...
    {
        $pwd_decode = '';
        $cmd = 'echo "' . $this->pwd . '" | base64 -d 2>&1';
        $ret = exec($cmd, $pwd_decode, $status);
        // 2017-11-30 windows下 exec执行出错
//        if($status != 0)
//        {
//            $error_msg = 'Server inner error';
//            return ERR_SERVER;
//        }
    ...
    private function mktemp()
    {
        ...
        $ret = exec($cmd.' 2>&1', $out, $status);
        // if ($status != 0)
        // {
        //     return null;
        // }
        // echo 'mktemp-';var_dump($out[0]);
        return $out[0];
    }
    ...
    public function genUserSig($sdkappid, $private_key_path)
    {
        // 创建临时sig文件
        $sig = $this->mktemp();
        // echo 'Account.php-genUserSig:';var_dump($sig);
        // if(!$sig) {
        //     return null;
        // }
        ...
        //     return null;
        // }
        // echo '<br/>Account.php-生成sig-';var_dump($out);
        // 读取sig
        $out=array();
        ...
        //     return null;
        // }
        // echo '<br/>Account.php-读取sig-';var_dump($out[0]);
        return $out[0];
    }
    ...
    public function register(&$error_msg)
    {
        ...
        try
        {
            ...
            // 加密密码
            $cmd = "echo $this->pwd | base64 2>&1";
            $pwd = '';
            exec($cmd, $pwd, $ret);
            // 2017-11-30 windows下 cmd执行 echo demo | base64 2>&1报错,linux可以
//            if($ret != 0)
//            {
//                $error_msg = 'Server inner error';
//                return ERR_SERVER;
//            }
            ...
            $result = $stmt->execute();
//            echo 'register->execute:'.print_r($result);
            if (!$result)
            {
