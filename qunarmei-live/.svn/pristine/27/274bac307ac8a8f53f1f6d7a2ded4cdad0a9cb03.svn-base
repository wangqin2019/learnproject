...
class Page
{
     ...
     * $listRows 每页记录数
     */
    public function __construct($sdata, $listRows=30,$key=''){
        $this->total=count($sdata);
        $this->listRows=$listRows;
        $this->uri=$this->getUri();
        $this->page=!empty($_GET["page"]) ? $_GET["page"] : 1;
        $this->pageNum=ceil($this->total/$this->listRows);
        $this->render=$this->pageHtml($key);
    ...
    private function first($key=''){
        ...
        if($this->page==1)
            ...
            $html.=" <a class='btn btn-primary-outline' href='{$this->uri}&page=1&key={$key} '>首 页</a>";

        return $html;
    }

    //上一页
    private function prev($key=''){
        $html = "";
        if($this->page==1)
            $html.=" <a class='current btn disabled'>上一页</a>";
        else
            $html.=" <a class='btn btn-primary-outline' href='{$this->uri}&page=".($this->page-1)."&key={$key}'>上一页</a>";
    ...
    private function next($key=''){
        ...
        if($this->page==$this->pageNum)
            ...
            $html.=" <a class='btn btn-primary-outline' href='{$this->uri}&page=".($this->page+1)."&key={$key}'>下一页</a>";

        return $html;
    }

    //尾页
    private function last($key=''){
        $html = "";
        if($this->page==$this->pageNum)
            $html.=" <a class='current btn disabled'>尾 页</a>";
        else
            $html.=" <a class='btn btn-primary-outline' href='{$this->uri}&page=".($this->pageNum)."&key={$key}'>尾 页</a>";
