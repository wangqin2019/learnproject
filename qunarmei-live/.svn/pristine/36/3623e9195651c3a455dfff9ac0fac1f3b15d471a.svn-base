...
class LiveModel extends Model
{
     ...
     * 根据搜索条件获取用户列表信息
     */
    public function getLivesByWhere($map, $Nowpage, $limits)
    {
        return $this->field('think_live.*')->where($map)->page($Nowpage, $limits)->order('id desc')->select();
    }
     ...
     * @param $where
     */
    public function getAllLives($where)
    {
        if($where)
        {
