...
//    protected $drawPic = 'http://ml.chengmei.com/jp0_0416.png';
     ...
     */
    public function goodsExtendAdd($goods_type)
    {
        ...
        $res_g = GoodsExtend::get($map);
        // 没有图片的
        if($res_g){
            $gd_name = $res_g[0]['name'];
            $arr['goods_name'] = $gd_name;
            $map_gd['title'] = ['like','%'.$gd_name.'%'];
            $map_gd['storeid'] = 0;
            $map_gd['deleted'] = 0;
            $res_gd = BjGoods::get($map_gd);
            if($res_gd){
                $gd_img = unserialize($res_gd['thumb_url']);
                $arr['goods_img'] = $gd_img;
            }
            $this->code = 1;
            $this->data = $arr;
            $this->msg = '获取成功';
