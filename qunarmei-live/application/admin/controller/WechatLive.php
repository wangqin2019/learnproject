...
class WechatLive extends Base
{
    ...
    public function configure(){
        ...
        if(request()->isAjax()){
            ...
            if($param['buy_begin'] && $param['buy_end']){
                ...
                $param['hide_time']=strtotime($param['hide_time']);
            }
        ...
        $info['hide_time']=$info['hide_time']?date('Y-m-d H:i:s',$info['hide_time']):'';
        ...
        $this->assign('liveStatus',config("live_status"));
        return $this->fetch();
    }
    ...
    public function live_user(){
        ...
        if($id!=''){
            ...
            $user=$room->getLiveByWhere($map,$platform_id);
            if(count($user)){
                $data=[];
                foreach ($user as $k => $v) {
                    ...
                    if($flag_rule && strlen($data[$k]['mobile'])>5){
    ...
    public function getBranch(){
        ...
        $allowBuyStore=Db::table('think_live_see_conf')->where('mobile',$mobile)->value('store_signs');
        if(count($allowBuyStore)){
            $allowBuyStoreId=Db::table('ims_bwk_branch')->where('sign','in',$allowBuyStore)->column('id');
            $allStoreDepart=Db::table('sys_departbeauty_relation')->where('id_beauty','in',$allowBuyStoreId)->column("CONCAT('d',id_department) id");
            $checkBranch=array_merge($allowBuyStoreId,$allStoreDepart);
            $departMent=Db::table('sys_department')->field("CONCAT('d',id_department) id,'0' pId,st_department name,st_department sign")->select();
            $branch=Db::table('sys_departbeauty_relation')->alias('r')->join(['ims_bwk_branch'=>'b'],'r.id_beauty=b.id','left')->field("id_beauty id,CONCAT('d',id_department) pId, CONCAT(title,sign) name,sign")->select();
            $branchList=array_merge($departMent,$branch);
            foreach($branchList as $k=>$v){
                $branchList[$k]['open']=$v['pId']?'true':'false';
                if(in_array($v['id'],$checkBranch)){
                    $branchList[$k]['checked']='true';
                }
            }
            $data=['storeSign'=>$allowBuyStore,'storeIds'=>implode(',',$allowBuyStoreId),'branchList'=>$branchList];
            return json(['code' =>1, 'data' =>$data, 'msg' => '主播暂未配置观看门店']);
        }else{
            return json(['code' =>0, 'data' => '', 'msg' => '主播暂未配置观看门店']);
        }
    }
