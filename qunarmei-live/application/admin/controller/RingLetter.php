...
class RingLetter extends Base
{
     ...
     * @author
     */
    public function sendMsg($chat_id,$msg,$fromer,$live_id,$type='')
    {
        $token = $this->getToken();
//        $headers = array('Authorization:Bearer'.$token,);
        $res = Db::name('ring_letter')->field('token,log_time,expires,more_users')->limit(1)->select();
        $msg_url = $this->url.$this->org_name.'/'.$this->app_name.'/messages';
        $data = '{"target_type" : "chatrooms","target" : ["'.$chat_id.'"],"msg" : {"type" : "txt","msg" : "'.$msg.'"},"from" : "'.$fromer.'","ext":{"intscroll":"'.$type.'"}}';
        if($type == 'ext')
        {
            //模拟聊天室水军发送消息 $userName
            $more_users = explode(',',$res[0]['more_users'])  ;
            $userName = $more_users[rand(0,24)];
            $data = '{"target_type" : "chatrooms","target" : ["'.$chat_id.'"],"msg" : {"type" : "txt","msg" : "'.$msg.'"},"from" : "'.$userName.'","ext":{"userName":"'.$userName.'"}}';
        }
...
//                }
//            }
            $data_v =array('live_id'=>$live_id,'chat_id'=>$chat_id,'fromer'=>$fromer,'msg'=>$msg,'request_msg'=>$data,'respone_msg'=>$rest,'log_time'=>date('Y-m:d H:i:s',time()));
    ...
    public function getChatrooms($id)
    {
        ...
        {
            //获取聊天室人数
            $rooms_url = $this->url.$this->org_name.'/'.$this->app_name.'/chatrooms';
            $rest = curl_get($rooms_url);
            if($rest)
            {
                $rest = json_decode($rest);
                $data = $rest->data;
