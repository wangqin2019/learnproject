...
//    protected $pid = [11,12,13,14,15,5036,8663,17354,17446,17454,17690,17701,17702,17715,17716,17717,17742,17743,17744,17780,17781,17782,21865,21866,21867,21868,21869,21870,21871,21872,28397,1746720,1748543,1748544,1748545,1751096,1751097,1751098];//要插入的父商品id
    ...
    public function index(){
        ...
        if($export){
            ...
            $header_arr = array('门店id','所属办事处','门店名称','门店编号','门店地址','412直播活动开关','直播消费券活动开关','520专属宠爱券活动');
            // CSV名称转中文试下
            ...
            for($a = 1;$a <= $num;$a++){
                ...
                if($lists1){
                    ...
                    foreach ($lists1 as $k1=>$v1) {
                        ...
                        $data1['statu_coupon520'] = $v1['statu_coupon520']==1?iconv('utf-8','gb2312//IGNORE','开'):iconv('utf-8','gb2312//IGNORE','关');
...
//            }
    ...
    {
        $id = input('id');
        $type = input('type',0);// 0:直播消费券活动,1:520专属宠爱券活动
        $arr = ['code'=>1,'msg'=>'修改成功'];
        // 查询状态
        $map['id'] = $id;
        ...
        if($res){
            ...
            if($type == 0){
                ...
                if($res['statu_coupon520']){
                    $data['statu_coupon520'] = 0;
                    $arr['code'] = 0;
                }else{
                    $data['statu_coupon520'] = 1;
                }
            }
