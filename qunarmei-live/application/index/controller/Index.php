...
//          $rest[0]['gk_cnt']=$this->getCnt($rest[0]['chat_id']);
             ...
             // 观看总人数
            $live_ser = new \app\api\controller\Live();
            $rest[0]['gk_cnt'] = $live_ser->getGkSum($rest[0]['chat_id'],$rest[0]['see_count_times']);

          if ($live_id == 18 && empty($mobile)) {
              $rest[0]['hls_url'] = $rest[0]['see_url'];
              $this->assign('rest', $rest[0]);
              return $this->fetch('guanwang');
          }
   ...
   public function zhibo_down()
   {
    ...
    public function getCnt($id)
    {
        ...
        $cnt = $live_ser->getGkSum($id,$res['see_count_times']);
        return $cnt;
//        if($id)
//        {
//            if($id == '@TGS')
...
//   PC端直播登录
    ...
    {
        session_start();
        $name =  @isset($_SESSION['zb'])?@$_SESSION['zb']:'';
        if($name)
        {
//          直播列表
...
//                    }
                    //获取观看点赞人数
//                    $v['gk_cnt'] = $this->getCnt($v['chat_id']);

                    $live_ser = new \app\api\controller\Live();
                    $v['gk_cnt'] = $live_ser->getGkSum($v['chat_id'],$v['see_count_times']);

                    $v['dz_cnt'] = $live_ser->getDzs($v['id']);
                }
            }
