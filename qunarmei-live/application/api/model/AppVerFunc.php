...
class AppVerFunc extends Model
{
    ...
    public static function cardSel($arr)
    {
        ...
        $map1 = " (u.ticket_code != '000000' or u.ticket_code is null) and (u.type < 18 or u.type >= 23)";
        $res = Db::table('pt_ticket_user u')->join(['ims_bj_shopn_member'=>'m'],['u.mobile=m.mobile'],'LEFT')->field('u.*')->where($map)->where($map1)->order('u.id desc')->select();
        return $res;
    }
}