...
//			$map['a.assess_name|p.assess_project'] = ['like' , '%'.$serach.'%'];
			$mapa['assess_name'] = ['like' , '%'.$serach.'%'];
			$mapp['assess_project'] = ['like' , '%'.$serach.'%'];
		}
		if($role == 1){
...
//			$res = $this->getAssessUserAssessProject($map);
			...
			$res = AssessUser::getAssessProject($mapu , $mapa , $mapp);
//			dump($res);die;
			if($res){
				// 考核名称,项目类型,项目名称,起止时间,提交时间,直播时间,考核状态
				foreach ($res as $v) {
//					dump($v['assess']['assess_name']);die;
					$arr1['assess_uid'] = $v['id'];
					$arr1['assess_name'] = $v['assess']['assess_name'];
					$arr1['assess_type'] = $v['assess']['assess_type'];// 1:直播,2:录像
					$arr1['assess_project'] = $v['assess']['project']['assess_project'];
					$arr1['begin_end_time'] = '';
					$arr1['submit_time'] = '';
					$arr1['live_time'] = '';
					$arr1['begin_end_time'] = date('Y.m.d H:i',$v['assess']['begin_time']).' 至 '.date('Y.m.d H:i',$v['assess']['end_time']);
					if($arr1['assess_type'] == 1){
...
//			$res = $this->getAssessProject($map);
			$res = Assess::getProject($mapa , $mapp);
			if($res){
				$ids = [];
				foreach ($res as $k => $v) {
					$res[$k]['num'] = 0 ;
				...
				foreach ($res as $v) {
					...
					$arr1['assess_project'] = $v['project']['assess_project'];
