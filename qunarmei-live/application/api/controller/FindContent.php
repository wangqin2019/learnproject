...
{
    // 点播分享H5观看url
    protected $shareUrl = 'http://live.qunarmei.com/index/index/pcplay';
    /*
     * 功能: 获取发现文章评论
     * 请求: $article_id=>文章id
     * 返回:
    ...
    public function get_comment()
    {
        ...
        {
            //查询文章
            $res1 = Db::name('find_content cnt')->join(['ims_bj_shopn_member'=>'mem'],['mem.id=cnt.user_id'],'left')->join(['ims_fans'=>'fans'],['mem.id=fans.id_member'],'left')->field('cnt.article_label,cnt.id article_id,cnt.user_id,cnt.article_img,cnt.article_title,cnt.article_content,cnt.comment_time,mem.realname user_name,fans.avatar user_img,cnt.article_video,cnt.type,cnt.article_jump_url,cnt.flag_img,mem.storeid')
                ->where('cnt.id='.$article_id)->limit(1)->select();
            if($res1)
            {
                $storeid_gd = $res1[0]['storeid'];
                unset($res1[0]['storeid']);
                // 内容为图片
                $res1[0]['content_img'] = [];
                if($res1[0]['flag_img']){
                    $mapimg['isshow'] = 1;
                    $mapimg['article_id'] = $res1[0]['article_id'];
                    $res_img = Db::name('find_content_img')->field('img_url,goods_id')->where($mapimg)->order('display_order desc')->select();
                    if($res_img){
                        $data_imgv = [];
                        foreach ($res_img as $v_img) {
                            $data_img['img_url'] = $v_img['img_url'];
                            $data_img['goods_id'] = $v_img['goods_id'];
                            // 根据父商品id查询子商品id
                            if($data_img['goods_id'] > 0){
                                $map1111['pid'] = $data_img['goods_id'];
                                $map1111['storeid'] = $storeid_gd;
                                $gd_id = Db::table('ims_bj_shopn_goods')->where($map1111)->limit(1)->value('id');
                                if(!empty($gd_id)){
                                    $data_img['goods_id'] = $gd_id;
                                }
                            }
                            $data_imgv[] = $data_img;
                        }
                        $res1[0]['content_img'] = $data_imgv;
                    }
                }
                $res1[0]['article_share_url'] = '';
                $res1[0]['article_share_url'] = config('share_content_url').'?article_id='.$res1[0]['article_id'];
                ...
                $res2 = Db::name('find_content_comment fcc')->join(['ims_bj_shopn_member'=>'mem'],['mem.id=fcc.user_id'],'left')->join(['ims_fans'=>'f'],['mem.id=f.id_member'],'left')->field('fcc.*,mem.realname user_name,f.avatar')->where('article_id='.$article_id)->order('comment_time desc')->limit($pre,$limits)->group('fcc.id')->select();
                ...
                if($res2)
                {
                    foreach($res2 as $v2)
                    {
                        ...
                        $data1['user_img'] = 'http://appc.qunarmei.com/normal_photo.png';
                        //查询用户头像
                        $res_tx = Db::table('ims_fans')->field('avatar')->where('id_member',$res2[0]['id'])->limit(1)->select();
                        if($v2['avatar']){
                            $data1['user_img'] = $v2['avatar'];
                        }
...
//                }
    ...
    public function get_content()
    {
        ...
        if(!$data1) {
            ...
            if ($key) {
                ...
                $res = Db::name('find_content cnt,ims_bj_shopn_member mem,ims_fans fans')->field('cnt.id article_id,cnt.article_label,fans.avatar user_img,cnt.user_id,mem.realname user_name,cnt.article_title,cnt.article_content,cnt.article_img,cnt.article_video,cnt.comment_time,cnt.article_label_color,cnt.type,cnt.article_jump_url')->where('cnt.isshow=1 and mem.id=fans.id_member and mem.id=cnt.user_id ' . $map)->group('cnt.id')->order('cnt.display_order desc,cnt.comment_time desc')->limit($pre, $limits)->select();
            }else
            {
                //查询文章数据
                $res = Db::name('find_content cnt,ims_bj_shopn_member mem,ims_fans fans')->field('cnt.id article_id,cnt.article_label,fans.avatar user_img,cnt.user_id,mem.realname user_name,cnt.article_title,cnt.article_content,cnt.article_img,cnt.article_video,cnt.comment_time,cnt.article_label_color,cnt.type,cnt.article_jump_url')->where('cnt.isshow=1 and mem.id=fans.id_member and mem.id=cnt.user_id ')->group('cnt.id')->order('cnt.display_order desc,cnt.comment_time desc')->limit($pre, $limits)->select();
            }
            if ($res) {
                foreach ($res as $v) {
    ...
    public function get_collet()
    {
        ...
        if($user_id)
        {
            ...
            $res = Db::name('find_content_collect fcc,think_find_content cnt,ims_bj_shopn_member mem,ims_fans fans')->field('cnt.id ,fans.avatar user_img,cnt.user_id,mem.realname user_name,cnt.article_title,cnt.article_img,cnt.comment_time,cnt.article_video,cnt.flag_img,cnt.cover_img')->where(' mem.id=fans.id_member and mem.id=cnt.user_id and fcc.actrile_id=cnt.id and fcc.type=1  and fcc.user_id='.$user_id)->limit($pre,$limits)->group('fcc.id')->order('cnt.comment_time desc')->select();
            if($res)
            {
                foreach($res as &$v)
                {
...
//                    $data1['article_img'] = $v['article_img'];
                    if(!empty($v['cover_img'])){
                        $data1['article_img']->img_url = $v['cover_img'];
                    }
                    ...
                    //resource_type=>1=>图片,2=>视频,3=>全是图片
    ...
    public function other_info($user_id='')
    {
        ...
        if(!$ret) {
            ...
            $res = Db::table('ims_bj_shopn_member mem')->join(['ims_bwk_branch'=>'ibb'],['mem.storeid=ibb.id'],'LEFT')->join(['ims_fans'=>'fan'],['mem.id=fan.id_member'],'LEFT')->field('fan.avatar,mem.realname,ibb.location_p')->where('mem.id='.$user_id)->limit(1)->select();
            if ($res) {
                //获取发布文章和被收藏数量
                ...
                $data1['img_tx'] = config('img.head_img');
                ...
                if($res4)
                {
                    foreach($res4 as $v)
                    {
                        ...
                        if(is_null($data2['article_img'])){
                            $data2['article_img'] = (object)[];
                        }
...
//                parent::setRedisP('other_info_'.$user_id,$data,3600*24);
    ...
    // 查询关注和粉丝数
    public function getGzFsNum($user_id)
    {
        $map['user_id'] = $user_id;
        $map['relation_type'] = 1;
        $res_gz = Db::table('think_follow')->where($map)->count();
        $map['follower_id'] = $user_id;
        unset($map['user_id']);
        $res_fs = Db::table('think_follow')->where($map)->count();
        $arr['follow_num'] = 0;
        $arr['fans_num'] = 0;
        if ($res_gz) {
            $arr['follow_num'] = $res_gz;
        }
        if ($res_fs) {
            $arr['fans_num'] = $res_fs;
        }
    ...
    public function anchor_info($user_id='')
    {
        ...
        if(!$ret) {
            ...
            if ($res) {
                ...
                $maps = 'missshop,missshop_transfer,missshop_exchange';
                $res_score = $scoreser->getUserSumScore($user_id,$maps);
                if($res_score){
                    $res1['scores'] = $res_score['sum_scores'];
                }
                /*
                 * 预留
                 * */
                ...
                $res2 = $this->getGzFsNum($user_id);
                $data1['gz_num'] = $res2['follow_num'];
                $data1['fs_num'] = $res2['fans_num'];
                $data = $data1;
//                parent::setRedisP('other_info_'.$user_id,$data,3600*24);
            }else {
                $msg = '暂无数据';
    ...
    /*
     * 功能: 个人中心-主播-资料
     * 请求: user_id=>用户id,,other_user_id=>其他用户id
     * 返回:
     * */
    public function anchor_info_own($user_id=0,$other_user_id=0)
    {
        //初始化设置
        $code = 1;$msg = '获取成功';$data = [];$data1=null;
        //获取redis里的数据
        ...
        if(!$ret) {
            ...
            $res = Db::table('ims_bj_shopn_member mem')->join(['ims_bwk_branch'=>'ibb'],['mem.storeid=ibb.id'],'LEFT')->join(['ims_fans'=>'fan'],['mem.id=fan.id_member'],'LEFT')->field('fan.avatar,mem.realname,ibb.location_p,mem.mobile')->where('mem.id='.$user_id)->limit(1)->select();
            if ($res) {
                $data1['img_tx'] = config('img.head_img');
                $data1['gz_num']='';$data1['fs_num']='';$data1['replay_video']=array();
                if(strstr($res[0]['avatar'],'http://'))
                {
                    $data1['img_tx'] = $res[0]['avatar'];
                }
                ...
                if($res[0]['mobile'])
                {
                    $res2 = Db::name('live')->field('id,insert_time,live_img,see_url,user_img,user_name,address')->where('statu=2 and live_source=2 and user_id='.$res[0]['mobile'])->order('insert_time desc')->select();
...
//                $res2 = Db::name('live')->field('id,insert_time,live_img,see_url')->where('user_id',$res[0]['mobile'])->order('insert_time asc')->select();
                    if ($res2) {
                        ...
                        $mapc['type'] = 1;
                        if ($other_user_id) {
                            $mapc['user_id'] = $other_user_id;
                        }else{
                            $mapc['user_id'] = $user_id;
                        }
                        $mapc['delete_time'] = 0;
                        $res_collect = Db::table('think_user_conf')->where($mapc)->select();
                        if ($res_collect) {
                            foreach ($res_collect as $kc => $vc) {
                                $collect_ids[] = $vc['content'];
                            }
                        }
                        foreach($res2 as $v)
                        {
                            $data2['live_id'] = $v['id'];
                            $data2['is_collet'] = 0;
                            if ($collect_ids && in_array($v['id'],$collect_ids)) {
                                $data2['is_collet'] = 1;
                            }
                            $data2['is_collect'] = $data2['is_collet'];
                            $data2['anchor_img'] = $v['user_img'];
                            $data2['anchor_name'] = $v['user_name'];
                            $data2['address'] = $v['address'];
                            //取redis里的观看数和点赞数
                            ...
                            $data2['share_url'] = $this->shareUrl.'?id='.$v['id'];
...
ranking,head_img,name,location,scores*/
    ...
    public function ranking_list_new()
    {
        /*2017年12月,12月
ranking,head_img,name,location,scores*/
        //请求数据
        ...
        $rest = [
            'dt' => [
                'year_m' => date('Y').'年'.date('m').'月',
                'month' => date('m').'月'
            ],
            'rank_list' => []
        ];

        $cacheSer = new CacheSer();
        $key = 'ranking_list_new_'.$user_id;
        $rank_list = $cacheSer->getCache($key);
        if($rank_list){
            $rest['rank_list'] = $rank_list;
            return $this->returnMsg($code,$rest,$msg);
        }

        $y = date('Y');$m=date('m');$data2='';$a=1;$flag=1;
        $res = Db::table('think_sum_user sum')->join(['ims_bj_shopn_member'=>'mem'],'sum.user_id=mem.id','LEFT')->join(['ims_fans'=>'fan'],'sum.user_id=fan.id_member','LEFT')->join(['ims_bwk_branch'=>'ibb'],'mem.storeid=ibb.id','LEFT')->field('sum.user_id,sum.scores,mem.realname,fan.avatar,ibb.location_p')->where('sum.scores>0')->order('sum.scores desc')->group('sum.user_id')->limit(100)->select();
        if($res)
        {
            foreach($res as $k=>$v)
            {
    ...
    public function upd_scores($arr=1)
    {
        ...
        if(is_array($arr))
        {
            $user_id = $arr['user_id'];$type = $arr['type'];$orderid = isset($arr['orderid'])?$arr['orderid']:'';
        }else
        {
                                    ...
                                    if($result_jf)
                                    {
                                        ...
                                        if($res_jf->code)
                                        {
                                            ...
                                            if($data_jf2)
                                            {
                                                ...
                                                $str_remark1 = $tishi_msg.',添加积分'.$jf_customer;
                                                $str_remark .= $scan_str;
    ...
    public function scores_info()
    {
        ...
        if($user_id)
        {
            $map['type'] = ['neq','missshop'];
            $res = Db::name('scores_record')->field('user_id,type,msg,scores,log_time')->where($map)->where('user_id',$user_id)->order('log_time desc')->select();
            if($res)
            {
                foreach ($res as $v) {
                    $data1['msg'] = $v['msg'];
    ...
    public function fashionGuide(){
        ...
        if($user_id && $store_id){
            $map['is_fashion'] = 1;
            $res = Db::name('find_content')->field('id,cover_img,article_title,summary')->where($map)->order('display_order desc')->select();
            if($res){
                foreach ($res as $v) {
                    $res1['article_id'] = $v['id'];
                    $res1['cover_img'] = $v['cover_img'];
                    $res1['article_title'] = '';
                    $res1['summary'] = '';
                    $data[] = $res1;
                }
    ...
    public function carousel(){
        ...
        if($user_id && $store_id){
            $map['isshow'] = 1;
            $res = Db::name('find_content_cruselpic')->field('id,article_id,cruselpic')->where($map)->order('display_order desc')->select();
            if($res){
                foreach ($res as $v) {
                    $res1['article_id'] = $v['article_id'];
                    $res1['cruselpic'] = $v['cruselpic'];
                    $data[] = $res1;
                }
            }
            if(empty($data)){
    ...
    public function cateList(){
        ...
        if($user_id && $store_id){
            $map['isshow'] = 1;
            $res = Db::name('find_content_category')->field('id,cate_title,store_ids,role_ids')->where($map)->order('display_order desc')->select();
            if($res){
                $rule_ser = new \app\api\service\RuleSer();
                foreach ($res as $v) {
                    $flag = $rule_ser->getSeeRule($user_id,$store_id,$v['store_ids'],$v['role_ids']);
                    $res1['cate_id'] = $v['id'];
                    $res1['cate_title'] = $v['cate_title'];
                    if($flag){
                        $data[] = $res1;
                    }
                }
    ...
    /*
     * 功能: 魅力风尚-分类下文章列表
     * 请求: user_id=>用户id,store_id=>门店id,key=>搜索,page=>当前页
     * */
    public function contentList(){
        ...
        if($user_id && $store_id){
            ...
            $pre = ($page-1)*$limits;
            $map['fc.isshow'] = 1;
            $map['fc.cate_id'] = $cate_id;
            $map['fc.article_title|fc.summary'] = ['like','%'.$key.'%'];
            $res = Db::name('find_content fc')->join(['ims_bj_shopn_member'=>'mem'],['mem.id=fc.user_id'],'LEFT')->join(['ims_fans'=>'fan'],['mem.id=fan.id_member'],'LEFT')->join(['think_sum_article'=>'sa'],['sa.article_id=fc.id'],'LEFT')->field('fc.store_ids,fc.role_ids,fc.id,fc.article_title,fc.summary,fc.cover_img,fan.avatar,mem.realname,sa.see_num,sa.comment_num,fc.cover_img_1,fc.flag_img')->where($map)->order('fc.display_order desc')->limit($pre,$limits)->select();
            if($res){
                $rule_ser = new \app\api\service\RuleSer();
                foreach ($res as $v) {
                    $flag = $rule_ser->getSeeRule($user_id,$store_id,$v['store_ids'],$v['role_ids']);
                    $res1['article_id'] = $v['id'];
                    $res1['article_title'] = $v['article_title'];
                    $res1['summary'] = $v['summary'];
                    $res1['cover_img'] = $v['cover_img_1']==null?$v['cover_img']:$v['cover_img_1'];
                    $res1['user_img'] = strstr($v['avatar'],'http')==null?$head_img:$v['avatar'];
                    $res1['user_name'] = $v['realname']==null?'':$v['realname'];
                    $res1['see_num'] = $v['see_num'];
                    $res1['comment_num'] = $v['comment_num'];
                    $res1['flag_img'] = $v['flag_img'];
                    if($flag){
                        $data[] = $res1;
                    }
                }
    ...
    // end Modify by wangqin 2018-10-17
    // start Modify by wangqin 2018-10-30
    /*
     * 分享api
     * 请求参数: user_id=>用户id,article_id=>文章id
     * */
    public function shareContent()
    {
        ...
        if($user_id && $article_id ){
            $map['id'] = $article_id;
            $res = Db::table('think_find_content')->field('id')->where($map)->limit(1)->find();
            if($res){
                $code=1;$msg='获取成功';
                $data['share_url'] = config('share_content_url').'?user_id='.$user_id.'&article_id='.$article_id;
            }else{
                $msg='该文章不存在';
            }
        }
