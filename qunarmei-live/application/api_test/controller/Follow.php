...
class Follow  extends Base
{
    ...
    /*
     * 功能: 关注用户列表/粉丝列表
     * 请求: user_id=>用户id;follower_id=>被关注者用户id;
     * 返回:
     * */
    public function follow_list()
    {
        ...
        }else
        {
            ...
            {
                //粉丝列表
                $res1 = Db::name('follow f,ims_bj_shopn_member mem,ims_fans fans')->field('f.user_id,fans.avatar user_img,mem.realname user_name,fans.address')->where(' mem.id=fans.id_member and f.user_id=mem.id and f.relation_type=1  and f.follower_id='.$user_id)->select();
                if($res1)
                {
                    $flag=1;
                    $data = $res1;
            ...
            {
                //关注列表
                $res1 = Db::name('follow f,ims_bj_shopn_member mem,ims_fans fans')->field('f.follower_id as user_id,fans.avatar user_img,mem.realname user_name,fans.address')->where(' mem.id=fans.id_member and f.follower_id=mem.id and f.relation_type=1 and f.user_id='.$user_id)->select();
                if($res1)
                {
                    $flag=1;
                    $data = $res1;
