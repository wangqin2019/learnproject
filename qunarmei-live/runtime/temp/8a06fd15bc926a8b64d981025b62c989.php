<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:86:"D:\software\phpstudy_pro\WWW\qunarmei-live\public/../application/admin\view\index.html";i:1574039644;s:84:"D:\software\phpstudy_pro\WWW\qunarmei-live\application\admin\view\public\header.html";i:1587693029;s:84:"D:\software\phpstudy_pro\WWW\qunarmei-live\application\admin\view\public\footer.html";i:1574039645;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div id="wrapper">
    <!-- 最上面多个系统导航开始 -->
    <div style="height: 60px;background-color: #4397e6">
        <div style="width: 220px;line-height:60px;float: left;height: 100%;text-align: center;font-size: 20px;color: #fff;font-weight: bold">   去哪美管理后台
        </div>
        <div style="width:calc(100% - 220px);line-height:60px;float: left;height: 100%">
            <div class="top_nav">
                <ul>
                    <?php if(is_array($menu) || $menu instanceof \think\Collection || $menu instanceof \think\Paginator): if( count($menu)==0 ) : echo "" ;else: foreach($menu as $key=>$vo): ?>
                    <li style="float: left;">
                        <a href="javasctipt:void(0);" alt="<?php echo $vo['id']; ?>" onclick="selNav('<?php echo $vo['id']; ?>',this)">
                           <?php echo $vo['title']; ?>
                        </a>
                    </li>
        ...
        <div class="sidebar-collapse">
            <ul class="nav" id="side-menu">
                <li class="nav-header">
                    ...
                    <div class="logo-element">去哪美
                    </div>
                </li>
                <?php if(!empty($menu)): if(is_array($menu) || $menu instanceof \think\Collection || $menu instanceof \think\Paginator): if( count($menu)==0 ) : echo "" ;else: foreach($menu as $key=>$vo): if(is_array($vo['child']) || $vo['child'] instanceof \think\Collection || $vo['child'] instanceof \think\Paginator): if( count($vo['child'])==0 ) : echo "" ;else: foreach($vo['child'] as $key=>$v): ?>
                        <li class="menu nav_<?php echo $vo['id']; ?>" style="display: block;">
                                ...
                                <ul class="nav nav-second-level">
                                    <?php if(!empty($v['child_son'])): if(is_array($v['child_son']) || $v['child_son'] instanceof \think\Collection || $v['child_son'] instanceof \think\Paginator): if( count($v['child_son'])==0 ) : echo "" ;else: foreach($v['child_son'] as $key=>$vv): ?>
                                        <li>
                                            <a class="J_menuItem" href="<?php echo $vv['href']; ?>"><?php echo $vv['title']; ?></a>
                                        </li>
...
function selNav(id,obj) {
    var thisObj=$(obj);//js对象转jquery对象
    $('.nav_'+id).show();
    $('#side-menu > li:gt(0)').not('.nav_'+id).hide();
    thisObj.parent().addClass('nav_action').siblings().removeClass('nav_action');
}
</script>
</body>
</html>
