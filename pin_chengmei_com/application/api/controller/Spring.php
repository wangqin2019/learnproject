...
class Spring extends Base
{
    ...
    public function check_activity(){
        $activityInfo=Db::name('activity_config')->where('id',9)->cache(86400)->find();
    ...
    public function goods_list(){
        ...
        $goods_id=input('param.goods_id',505);
        $map['id']=array('eq',$goods_id);
        ...
        if(count($goodsData)){
            foreach ($goodsData as $k=>$v){
                ...
                $goodsData[$k]['stock']=$this->get_stock($v['id'],$v['stock']);
                $goodsData[$k]['given']=$this->get_given($v['given'],$v['id']);
                unset($goodsData[$k]['recommend_price'],$goodsData[$k]['model_id'],$goodsData[$k]['pid']);
            }
        }
        if($goodsData){
            $code = 1;
    ...
    public function buy(){
        ...
        if($uid) {
            ...
            //合并产品订单和配赠
            $arr = array('uid' => $uid, 'storeid' =>$storeid, 'fid' => $sellerId, 'share_uid' => $fid, 'num' => $total_goods+$give_total_goods, 'order_sn' => $ordersn, 'order_price' => $total_price, 'pay_price' => $total_price, 'channel' => 'missshop', 'pick_type' => 1, 'insert_time' => time(), 'pid' => '', 'scene' => 7, 'flag' => 3, 'specs' => '');
    ...
    public function get_given($given,$pid){
        ...
        if(strlen($given)){
            $m['status']=array('eq',1);
            $m['id']=array('in',$given);
            $goods_given=Db::name('goods')->where($m)->field('id,name,goods_code,image,stock')->select();
        }else{
