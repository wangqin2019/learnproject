 ...
 * desc:代餐活动新版
 */
class SubstituteMeal extends Base
{
    protected $aid;
    protected $actInfo;
            ...
            if($mobile) {
                ...
                if (strlen($good_list)) {
                    foreach ($goods_list as $k => $v) {
                        ...
                        $orderInfo[] = ['order_sn' => $ordersn, 'good_id' => $params[0], 'good_num' => $params[2], 'good_specs_sku' => $params[1], 'good_price' => $goods['activity_price'], 'good_amount' => $amount, 'main_flag' => 1, 'good_specs' => $specs, 'source' => 0, 'flag' => 0, 'insert_time' => time(),'give_cate'=>''];
                        $total_price += $amount;
                ...
                if (strlen($give_list)) {
                    foreach ($gives_list as $kk => $vv) {
                        ...
                        $goods = Db::name('goods')->where('id', $params[0])->find();
                        $amount = $goods['activity_price'] * $params[2];
                        $giveInfo[] = ['order_sn' => $ordersn, 'good_id' => $params[0], 'good_num' => $params[2], 'good_specs_sku' => $params[1], 'good_price' => $goods['activity_price'], 'good_amount' => $amount, 'main_flag' => 0, 'good_specs' => $specs, 'source' => 0, 'flag' => 1, 'insert_time' => time(),'give_cate'=>$this->actInfo['give_cate']];
                        $give_total_goods += $params[2];
                    }
                }
                $orderList = array_merge($orderInfo, $giveInfo);
                //合并产品订单和配赠
                $arr = array('uid' => $uid, 'storeid' => $storeid, 'fid' => $sellerId, 'share_uid' => $fid, 'num' => $total_goods + $give_total_goods, 'order_sn' => $ordersn, 'order_price' => $total_price, 'pay_price' => $total_price, 'channel' => 'missshop', 'pick_type' => 1, 'insert_time' => time(), 'pid' => '', 'scene' => 18, 'flag' => 5, 'specs' => '','remark'=>'');
