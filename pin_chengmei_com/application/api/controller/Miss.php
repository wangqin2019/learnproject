...
//        }
    ...
    public function orderList(){
        ...
        if($uid!=''){
            ...
            $list = Db::name('activity_order')->alias('o')->join(['ims_bj_shopn_member' => 'm'],'o.uid=m.id','left')->join('wx_user u','m.mobile=u.mobile','left')->join('goods p','o.pid=p.id','left')->field('uid,fid,o.pid,pay_status,order_status,pay_price,order_price,order_sn,u.avatar,u.nickname,num,pick_code,pick_type,o.specs,p.name p_name,p.unit,m.isadmin,m.code,o.is_axs')->where($map)->page($Nowpage, $limits)->order('o.id desc')->select();
            if(count($list) && is_array($list)){
                foreach ($list as $k=>$v){
                    if($v['pid']==47 || $v['pid']==79 || $v['pid']==95){
						 $list[$k]['allow_draw']=0;
                        $list[$k]['lucky_flag']=0;
                        $list[$k]['lucky_draw']='';
                    }else{
