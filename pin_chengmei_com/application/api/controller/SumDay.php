...
//            print_r($arr2);die;
    ...
    // 每日统计分享购成交金额数量和单独购成交金额数量
    // $dt1=>昨天日期时间戳,$dt2=>今天日期时间戳
    public function sumBuyPrice($dt1,$dt2){
        $store_ids = [];
        $dt11 = date('Y-m-d',$dt1);// 时间戳转换成日期
        // 分享购每日成交金额
        $map['order_type'] = 1;
        $map['l.success_time'] = ['>=',$dt1];
        $map1['l.success_time'] = ['<',$dt2];
        ...
        if($res){
            $data2['buy_share_price_day']=0;
            $arr1['dt1'] = $dt11;
            foreach ($res as $v) {
                $arr1['storeid'] = $v['storeid'];
                $data1['buy_share_price_day'] = $v['pay_price'];
                // 修改每个门店分享购每日成交金额
                $this->upd_day_data($arr1,$data1);
                $data2['buy_share_price_day'] += $data1['buy_share_price_day'];
            }
            // 修改总门店分享购每日成交金额
            $this->upd_day_data_all($arr1,$data2);
        }
        // 单独购每日成交金额
        $data2 = [];$data1=[];
        $map['order_type'] = 2;
        ...
        $res = Db::name('tuan_info ti')->join(['pt_tuan_list'=>'l'],'l.tuan_id=ti.id','LEFT')->where("l.status=2")->field('sum(l.tuan_price) pay_price,l.storeid,l.order_type')->where($map)->where($map1)->group('l.storeid')->order('l.storeid desc')->select();
        if($res){
            $data2['buy_alone_price_day'] = 0;
            $arr1['dt1'] = $dt11;
            foreach ($res as $v) {
                $arr1['storeid'] = $v['storeid'];
                $data1['buy_alone_price_day'] = $v['pay_price'];
                // 修改每个门店单独购每日成交金额
                $this->upd_day_data($arr1,$data1);
                $data2['buy_alone_price_day'] += $data1['buy_alone_price_day'];
            }
            // 修改总门店单独购每日成交金额
            $this->upd_day_data_all($arr1,$data2);
        }
    ...
    // 修改每天门店分享购和单独购新顾客人数
    // dt1=>昨日日期时间戳;dt2=>今日日期时间戳
    public function xgkBuy($dt1,$dt2){
        // 昨日日期
        $dt3 = date('Y-m-d',$dt1);
        $data2['buy_share_xgk_day']=0;$data4['buy_alone_xgk_day']=0;
        $res1 = $this->sel_pay_stores($dt1,$dt2);
        if($res1){
            foreach ($res1 as $v1) {
                $map11['l.status'] = 2;
                $map11['l.order_type'] = 1;
                $map11['l.storeid'] = $v1['storeid'];
                $map11['o.pay_time'] = ['>=',$dt1];
                $map12['o.pay_time'] = ['<',$dt2];
                $map11['m.id_regsource'] = 7;
                $where11 = ' o.order_sn!=o.parent_order ';
                // 分享购
                $res = Db::name('tuan_list l')->join(['pt_tuan_order'=>'o'],'l.order_sn=o.parent_order','LEFT')->join(['ims_bj_shopn_member'=>'m'],'m.id=o.uid','LEFT')->field('count(o.uid) cnt,m.id_regsource,l.storeid')->where($map11)->where($map12)->where($where11)->group('m.id_regsource')->limit(1)->find();
                if($res){
                    $arr1['storeid'] = $v1['storeid'];
                    $arr1['dt1'] = $dt3;
                    $data1['buy_share_xgk_day'] = $res['cnt'];
                    $data2['buy_share_xgk_day'] += $data1['buy_share_xgk_day'];
                    $this->upd_day_data($arr1,$data1);
                }
                // 单独购
                $map11['l.order_type'] = 2;
                $where11 = ' o.order_sn=o.parent_order ';
                $res2 = Db::name('tuan_list l')->join(['pt_tuan_order'=>'o'],'l.order_sn=o.parent_order','LEFT')->join(['ims_bj_shopn_member'=>'m'],'m.id=o.uid','LEFT')->field('count(o.uid) cnt,m.id_regsource,l.storeid')->where($map11)->where($map12)->where($where11)->group('m.id_regsource')->limit(1)->find();
                if($res2){
                    $arr1['storeid'] = $v1['storeid'];
                    $arr1['dt1'] = $dt3;
                    $data3['buy_alone_xgk_day'] = $res2['cnt'];
                    $data4['buy_alone_xgk_day'] += $data3['buy_alone_xgk_day'];
                    $this->upd_day_data($arr1,$data3);
                }
            }
            // 修改总门店分享购每日新顾客
            $arr1['dt1'] = $dt3;
            $this->upd_day_data_all($arr1,$data2);
            // 修改总门店单独购每日新顾客
            $this->upd_day_data_all($arr1,$data4);
        }
    }

    // 查询昨天有支付订单的门店
    // $dt1=>昨日日期时间戳,$dt2=>今日日期时间戳,
    private function sel_pay_stores($dt1,$dt2){
        $map1['l.status'] = 2;
        $map1['l.success_time'] = ['>=',$dt1];
        $map2['l.success_time'] = ['<',$dt2];
        $res = Db::name('tuan_list l')->field('DISTINCT l.storeid')->where($map1)->where($map2)->group('storeid')->select();
        return $res;
    }

    // 修改每天门店数据
    // $arr1=>storeid[门店id],dt1=>今日日期; $data1=>修改数据
    private function upd_day_data($arr1,$data1){
        $res = Db::name('sum_visit_day')->where("storeid=".$arr1['storeid']." and log_time='".$arr1['dt1']."'")->update($data1);
        return $res;
    }
    // 修改每天总门店数据
    // $arr1=>storeid[门店id],dt1=>今日日期; $data1=>修改数据
    private function upd_day_data_all($arr1,$data1){
        $res = Db::name('sum_visit_day')->where("storeid=0 and log_time='".$arr1['dt1']."'")->update($data1);
        return $res;
    }
}