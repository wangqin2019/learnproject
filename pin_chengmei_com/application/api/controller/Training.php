...
//            }
    ...
    public function user_record(){
        ...
        if($uid !=''){
            ...
            //如果加入时间是21天 且今天已经打过卡了 就说明 他已经结业了  弹出结业证书
            if($joinDay==21){
                if($res['is_daka']){
                    $getPic=Db::name('training_member') ->where(['uid'=>$uid])->value('certificate');
                    if(strlen($getPic)>1){
                        $res['is_end']=1;
                        $res['certificate_pic']=$getPic;
                    }else{
                        $res['is_end']=0;
                        $res['certificate_pic']='';
                    }
    ...
    public function update_certificate(){
        ...
        $pic=input('param.pic');
        if($uid!=''){
            Db::name('training_member')->where('uid',$uid)->update(['certificate'=>$pic]);
            $code = 1;
            $data = '';
            $msg = '提交成功';
        }else{
...
//                }elseif($continued==7){
//                    $str='恭喜您！连续打卡'.$continued.'天，继续打卡满7天可再获得免费代餐。';
//                }elseif($continued>=8 && $continued<14){
//                    $str='恭喜您！连续打卡'.$continued.'天，距离免费代餐只剩'.(21-($continued+7)).'天';
//                }elseif($continued==14){
//                    $str='恭喜您！连续打卡'.$continued.'天，继续打卡满7天可再获得免费代餐和长筒袜。';
//                }elseif($continued==21){
//                    $str='恭喜您！连续打卡'.$continued.'天，请联络您的专属美容顾问领取大礼！';
//                }else{
//                    $str='恭喜您！连续打卡'.$continued.'天，免费代餐和夜间长筒袜只剩'.(21-$continued).'天';
//                }
                $str='恭喜您！连续打卡'.$continued.'天，继续保持哦';
                $code = 1;
                $data = $str;
                $msg = '提交成功';
            }catch (\Exception $e){
