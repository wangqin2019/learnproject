...
//        $res = $this->getGkDay(strtotime($dt1),$hour2);
        $this->updDataFlag($dt1,$hour2);
        print_r('visitHour1-updDataFlag');die;
        $this->code = 1;
        $this->msg = '每小时数据更新成功';
        // 记录日志
        $this->write_log(['msg'=>'每小时定时任务-'.json_encode($this->data,JSON_UNESCAPED_UNICODE)]);
...
//        $customer_old = count($gk_2);
    ...
    public function getCjRate($dt1=null,$dt2=null,$hour=null){
        ...
        if(!empty($brr)){
            foreach ($brr as $vb) {
                $data_sm['month_price_rate'] = isset($vb['month_price_rate'])?$vb['month_price_rate']:0;
                $data_sm['month_num_rate'] = isset($vb['month_num_rate'])?$vb['month_num_rate']:0;
    ...
    public function getGkRate($dt1=null,$dt2=null,$hour=null){
        ...
        if(!empty($brr)){
            foreach ($brr as $vb) {
                $data_sm['month_xgk_rate'] = isset($vb['month_xgk_rate'])?$vb['month_xgk_rate']:0;
                $data_sm['month_lgk_rate'] = isset($vb['month_lgk_rate'])?$vb['month_lgk_rate']:0;
