...
//                        break;
   ...
   public function pickUpConfrim(){
        ...
        if($qrcode_type==7){//$qrcode_type等于7兑换推广积分，但下面有一个orderSn为必填，所以造一个假数值，通过验证
            $orderSn='123';
        }
        if($orderSn!='') {
            try {
                switch ($qrcode_type){
                    ...
                    case 2:
                        ...
                        if ($getOrderInfo) {
                            ...
                            }else if($getOrderInfo['flag']==2 || $getOrderInfo['flag']==3 || $getOrderInfo['flag']==4 || $getOrderInfo['flag']==5){
                                if ($getOrderInfo['order_status'] == 1) {
                                    return parent::returnMsg(0,'','请勿重复确认');
                                } else {
                                    Db::name('activity_order')->where('order_sn', $getOrderInfo['order_sn'])->update(['order_status' => 1]);
                                    Db::name('activity_order_info')->where('order_sn', $getOrderInfo['order_sn'])->update(['pick_up' => 1,'update_time'=>time()]);
                                    return parent::returnMsg(1,'','确认成功');
                                }
                            }else{
                                if ($getOrderInfo['order_status'] == 1) {
                    ...
                    case 6:
                        ...
                        break;
                    case 7:
                        $exchange=input('param.score',0);
                        $score_uid=input('param.score_uid');
                        if(!$score_uid){
                            return parent::returnMsg(0,'','错误，请重试！');
                        }
                        if(!$exchange){
                            return parent::returnMsg(0,'','请填写正确的兑换分值！');
                        }
                        $score=Db::name('promoter')->alias('p')->where('user_id',$score_uid)->sum('money');
                        if($exchange > $score){
                            return parent::returnMsg(0,'','您的可兑换积分不满足！');
                        }

                        $sellerInfo=Db::table('ims_bj_shopn_member')->where('id',$score_uid)->field('staffid,storeid')->find();
                        $scoreData = array('user_id' => $score_uid, 'fid'=>$sellerInfo['staffid'],'goods_id'=>0,'storeid'=>$sellerInfo['storeid'],'order_sn'=>'','remark' => '兑换'.$score.'积分', 'money' => -$exchange, 'type' => '88福袋', 'action_uid'=>$uid,'insert_time' => date('Y-m-d H:i:s', time()));
                        Db::name('promoter')->insert($scoreData);
                        return parent::returnMsg(1,'','兑换成功');
                        break;
                    default:
            ...
            if(count($order)>1){
                ...
                switch ($order[0]){
                    case 'missshop':
                        ...
                        if (is_array($orderInfo) && count($orderInfo)) {
                            if($sellerStoreId==$orderInfo['storeid']) {
                                if($orderInfo['flag']==1 || $orderInfo['flag']==4){
                                ...
                                }elseif($orderInfo['flag']==2){
                                    if ($orderInfo['order_status'] == 1) {
                                        return parent::returnMsg(0,'','该订单已取货');
                                    } else {
                                        $orderInfo['name'] ='与你同行抗疫法宝产品';
                                        $orderInfo['insert_time'] = date('Y-m-d H:i:s', $orderInfo['insert_time']);
                                        $orderInfo['pay_time'] = date('Y-m-d H:i:s', $orderInfo['pay_time']);
                                        $orderInfo['qrcode_type'] = 2;
                                        $orderInfo['unit'] = '件';
                                        $orderInfo['image'] = 'https://pgimg1.qunarmei.com/kyfb.jpg';
                                        $orderInfo['images'] = 'https://pgimg1.qunarmei.com/kyfb.jpg';
                                        $orderInfo['good_id'] = '';
                                        unset($orderInfo['pid'],$orderInfo['flag']);
                                        return parent::returnMsg(1,$orderInfo,'获取成功');
                                    }
                                }elseif($orderInfo['flag']==3){
                                    if ($orderInfo['order_status'] == 1) {
                                        return parent::returnMsg(0,'','该订单已取货');
                                    } else {
                                        $orderInfo['name'] ='约惠春天女王独宠产品';
                                        $orderInfo['insert_time'] = date('Y-m-d H:i:s', $orderInfo['insert_time']);
                                        $orderInfo['pay_time'] = date('Y-m-d H:i:s', $orderInfo['pay_time']);
                                        $orderInfo['qrcode_type'] = 2;
                                        $orderInfo['unit'] = '件';
                                        $orderInfo['image'] = 'https://pgimg1.qunarmei.com/spring.jpg';
                                        $orderInfo['images'] = 'https://pgimg1.qunarmei.com/spring.jpg';
                                        $orderInfo['good_id'] = '';
                                        unset($orderInfo['pid'],$orderInfo['flag']);
                                        return parent::returnMsg(1,$orderInfo,'获取成功');
                                    }
                                }elseif($orderInfo['flag']==5){
                                    if ($orderInfo['order_status'] == 1) {
                                        return parent::returnMsg(0,'','该订单已取货');
                                    } else {
                                        $orderInfo['name'] ='免疫力专区产品';
                                        $orderInfo['insert_time'] = date('Y-m-d H:i:s', $orderInfo['insert_time']);
                                        $orderInfo['pay_time'] = date('Y-m-d H:i:s', $orderInfo['pay_time']);
                                        $orderInfo['qrcode_type'] = 2;
                                        $orderInfo['unit'] = '件';
                                        $orderInfo['image'] = 'https://pgimg1.qunarmei.com/mylzq.png';
                                        $orderInfo['images'] = 'https://pgimg1.qunarmei.com/mylzq.png';
                                        $orderInfo['good_id'] = '';
                                        unset($orderInfo['pid'],$orderInfo['flag']);
                                        return parent::returnMsg(1,$orderInfo,'获取成功');
                                    }
                    ...
                    case 'sharing':
                        ...
                        break;
                    case 'promoter':
                        $map['user_id']=array('eq',$order[1]);
                        $map['type'] = array('eq', '88福袋');
                        $score=Db::name('promoter')->where($map)->sum('money');
                        $userInfo=Db::table('ims_bj_shopn_member')->alias('m')->join('wx_user u','m.mobile=u.mobile','left')->where('m.id',$order[1])->field('u.nickname,u.avatar as images,m.storeid,m.staffid')->find();
                        if (is_array($userInfo) && count($userInfo)) {
                            $userInfo['score']=$score;
                            $userInfo['score_uid']=$order[1];
                            $userInfo['qrcode_type'] = 7;
                            if($sellerStoreId==$userInfo['storeid']) {
                                return parent::returnMsg(1, $userInfo, '获取成功');
                            }else{
                                return parent::returnMsg(0, '', '您无权限处理');
                            }
                        } else {
