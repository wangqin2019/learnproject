...
class Bargain1 extends Base
{
    ...
    public function goods_list() {
        $storeid = intval( input('param.storeid', 0) );//门店
        $user_id = intval( input('param.user_id', 0) );//用户
        ...
        if(!empty($list)){
            ...
            foreach ($list as $k=>$val){
                $deputy_cate = $val['deputy_cate'];
                $goods_id = $val['goods_id'];
                if($deputy_cate == 3){
                   $list[$k]['isbuy'] = $goodsBargainModel->checkCurrentUserBuyChance($storeid,$user_id,$goods_id);
                }else{
                    $list[$k]['isbuy'] = true;
                }
            }
        ...
        if(!empty($list)){
            return parent::returnMsg(1,$info,'获取成功');
        }else{
            return parent::returnMsg(0,'','暂无数据');
        }
    }
    /**
    ...
    public function goods() {
        //$this->transfer();
        $storeid = intval( input('param.storeid', 0) );//门店
        $user_id = intval( input('param.user_id', 0) );//登录用户
        if(empty($storeid)){
            return parent::returnMsg(0,'','门店未参与活动或已关闭');
        }
        ...
        if(empty($list)){
            return parent::returnMsg(0,'','商品不存在或已下架');
        }
        $info['list'] = $list;
        ...
        //1 主活动  2 特殊定制  3 2019裂变
        $info['storePlan'] = $storePlan;
        if($list['deputy_cate'] == 3){
            $info['isbuy'] = $goodsBargain->checkCurrentUserBuyChance($storeid,$user_id,$goods_id);
        }

        return parent::returnMsg(1,$info,'商品信息获取成功');
    }
    /**
    ...
    public function share_goods() {
        ...
        try{
            ...
            //老客裂变2位新客好友获取一次发起活动机会
            if($list['deputy_cate'] == 3){
                //用户是否有购买该商品的机会
                $flag = $GoodsBargainModel->checkCurrentUserBuyChance($storeid,$uid,$goods_id);
                if(!$flag){
                    return parent::returnMsg(404,'','您有订单未支付或已失效，暂无法参与该商品的活动');
                }
            }
            $time = time();
            //生成订单
            ...
            $orders['fid'] = $fid;
            $orders['flag'] = $list['deputy_cate'];//订单标识 1主活动商品订单 2、14家门店特殊定制 3、2019年终裂变活动
        ...
        try {
            if($buyType == 1){
                ...
                if($pay_end_time <= time()){
                    return parent::returnMsg(0,'','订单已超时，下单时间：'.date('Y-m-d H:i:s',$order_begin_time).'，有效时长：' .$this->config['duration'].'h');
                }
