...
class GoodsCateModel extends Model
{
     ...
     * @param $param
     */
    public function insertGoodsCate($param)
    {
        try{
            $result =  $this->validate('GoodsCateValidate')->allowField(true)->save($param);
            if(false === $result){
    ...
    {
        try{
            $result =  $this->validate('GoodsCateValidate')->allowField(true)->save($param, ['id' => $param['id']]);
            if(false === $result){
                return ['code' => 0, 'data' => '', 'msg' => $this->getError()];
            }else{
                return ['code' => 1, 'data' => '', 'msg' => '编辑产品分类成功'];
            }
        }catch( PDOException $e){
    ...
    public function delGoodsCate($id)
    {
        try{
            ...
            return ['code' => 1, 'data' => '', 'msg' => '删除产品分类成功'];
        }catch( PDOException $e){
            return ['code' => 0, 'data' => '', 'msg' => $e->getMessage()];
        }
    }
