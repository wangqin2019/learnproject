...
class Box extends Base
{
    ...
    //新年上上签鼠卡
    public static $new_year_rats = [
        1 => [
            'id' => 1,
            'cid' => 2,
            'name' => '桃花鼠',
            'thumb' => 'https://pgimg1.qunarmei.com/2019/12/e0d99201912311052011804.png',
            'thumb1' => 'https://pgimg1.qunarmei.com/2019/12/dffa4201912311052053511.png',
            'intro' => '桃花满满，心有所“鼠”',
            'type' => 0
        ],
        2 => [
            ...
            'cid' => 2,
            'name' => '吃货鼠',
            'thumb' => 'https://pgimg1.qunarmei.com/2019/12/16567201912311052188691.png',
            'thumb1' => 'https://pgimg1.qunarmei.com/2019/12/d6407201912311052227208.png',
            'intro' => '唯有美食与爱不可辜负的吃货鼠',
            'type' => 0
        ],
        3 => [
            'id' => 3,
            'cid' => 2,
            'name' => '机智鼠',
            'thumb' => 'https://pgimg1.qunarmei.com/2019/12/4edc3201912311052339164.png',
            'thumb1' => 'https://pgimg1.qunarmei.com/2019/12/97eae201912311052367855.png',
            'intro' => '小聪明与大智慧兼备的学霸机智鼠',
            'type' => 0
        ],
        4 => [
            'id' => 4,
            'cid' => 2,
            'name' => '欢脱鼠',
            'thumb' => 'https://pgimg1.qunarmei.com/2019/12/1a7a9201912311052498593.png',
            'thumb1' => 'https://pgimg1.qunarmei.com/2019/12/f6e7a201912311052525517.png',
            'intro' => '欢脱小福鼠，逍遥又自在',
            'type' => 0
        ],
    ];
    ...
    public function _initialize() {
        ...
        if(empty($blink_box_card_number) ){
            /*$blink_box_card_number = Db::name('blink_box_card')->where('status',0)->count();
            //卡片剩余数量
            $this->setCacheString('blink_box_card_number',intval($blink_box_card_number));
            if($blink_box_card_number <= 0){
                echo json_encode(array('code'=>400,'data'=>'','msg'=>'活动已结束!'));
                exit;
            }
            ...
            $blink_box_goods = GoodsModel::where($param)
            ...
            foreach ($blink_box_goods as $k=>$v){
                //设置每种产品的库存
                $this->setCacheString('blink_box_goods_stock_'.$k,intval($v['stock']));
            }
            //产品个数
            $this->setCacheString('blink_box_goods_number',count($blink_box_goods));
            //设置盒子库存
    ...
    public function accept_blink(){
        ...
        $boxs = Db::name('blink_order_box')
            ->where('blinkno',$give_num)
            ->where('uid',$give_userid)
        ...
        $goods = BlinkOrderBoxModel::alias('bob')
            ...
            ->where('bob.blinkno',$info['blinkno'])
            ->where('bob.uid',$info['give_uid'])
    ...
    public function give_card(){
        ...
        try{
            ...
            $param['is_give'] = 0;//未赠送
            //检测卡片数据是否存在  随机查询一条数据
            $info = Db::name('blink_order_box_card')->where($param)->find();
            if(empty($info)){
    ...
    public function pickUpGoodsInfo(){
        ...
        switch ($order[0]){
            case 'blinkcompose':
                //核销2020大礼包 19张100元 1张120元
                //检测卡券商品是否存在
                $coupon = Db::name('blink_box_coupon_user')->where('ticket_code',$order[1])->find();
                if(empty($coupon)){
                    return parent::returnMsg(0,'','卡券商品不存在或已失效，请确认');
                }
                if($coupon['source'] != 3){
                    return parent::returnMsg(0,'','卡券商品类型错误，请确认');
                }
                //检测卡券用户的门店
                $storeid = Db::table('ims_bj_shopn_member')->where('id',$coupon['uid'])->value('storeid');
                //同一门店可以核销
                if($storeid != $sellerStoreId){
    ...
    public function newyear(){
        ...
        try {
            ...
            if(empty($newyser)){
                //获取随机内容对应的鼠卡
                $rand_id = mt_rand(1,4);
                //$rats = self::$new_year_rats[$rand_id];
                /*$rats = Db::name('blink_box_card_image')
                    ->where('id',$rand_id)
                    ->find();*/
                //添加记录
